...
(function () {
    ...
    var IssueList = function IssueList() {
        ...
        MashupPlatform.wiring.registerCallback("inputSelectIssue", handlerInputSelectIssue.bind(this));
...
/**************************************************************************/
    ...
    var createTable = function createTable() {
        var columns = [
            {field: "id", label: "#", width: "65px", sortable: true},
            ...
            {field: "closingDate", label: "Closing Date", width: "160px"},
            {field: "affectedId", label: "Resource", width: "165px", sortable: true},
        ...
        this.table = new StyledElements.ModelTable(columns, {id: 'id', pageSize: 3});
    ...
    var displayInitialMessage = function displayInitialMessage() {
        ...
        title.textContent = "This widget needs issues.";
        textContainer.setAttribute("class", "p1InfoBox");
        text.textContent = "Get real time issues and assign to your technician staff, please, wire this widget to a data source (i.e. pub/sub broker).";
...
/**************************************************************************/
    ...
    var handlerInputSelectIssue = function handlerInputSelectIssue(issueString) {
        var issue = JSON.parse(issueString);
    ...
    var createSendRoute = function createSendRoute(issue, technician) {
        ...
        if (technician) {
            route.from = this.technicians[technician].getValue('FN', 0);
            route.to = issue.affectedId;
        } else {
            route.from = "";
            route.to = "";
        ...
        MashupPlatform.wiring.pushEvent('outputRoute', routeString);
    };
